...
export default function Mlbb({type}) {
    ...
    const btn_post=(e)=>{
        ...
        if(lenProduct.length <= 6){
            if(title != "" && description != "" && value != "" && email != "" && email_password != "" && username!="" && password!="" && imageList != "" ){
                e.preventDefault();
                handleshow2();
        
                const data = {title,description,value,email,email_password,username,password,type};
  ...
  return (
    <div>
        <form className='form_type_game'>
            ...
            <div id="post_photo">
                {isloading?<ClipLoader color={'#360bf7'} cssOverride={override} loading={isloading} size={100} aria-label="Loading Spinner" data-testid="loader"/>
                :imageList.map((url)=>{
            ...
            <hr size="5" color='black' />
            <h4>Account Information : Moonton</h4>

            <div id='gt_acc'>
                <p className='p'>Email :</p>
                <input value={email} onChange={e=>setemail(e.target.value)} className='coc_ip' id='coc_ip' type='text' placeholder='Email...'/>

                <p className='p'>Email Password :</p>
                <input value={email_password} onChange={e=>setemail_password(e.target.value)} className='coc_ip' id='coc_ip' type='password' placeholder='Password...'/>
            </div>

            <div className='mlbb_gmail' id='gt_gmail'>
                <p className='p' id='mlbb_p'>Moonton Account or Username :</p>
                <input value={username} onChange={e=>setusername(e.target.value)} className='coc_ip' id='coc_ip' type='text' placeholder='Username...'/>

                <p className='p'>Password :</p>
                <input value={password} onChange={e=>setpassword(e.target.value)} className='coc_ip' id='coc_ip' type='password' placeholder='Password...'/>
            </div>
